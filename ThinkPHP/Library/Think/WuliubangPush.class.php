...
class WuliubangPush{
    ...
    public function push_new_goods($goods_id=131,$operation=1){
        //推送出发地
        $this->push_goods($goods_id,$operation);
        //推送目的地
        $this->push_goods($goods_id,$operation,1);
    }
    ...
    public function push_car_message($car_info,$goods_id,$operation=1,$push_state){
        ...
        foreach($car_info as $v){
            ...
            $hz_id = $this->goods_table->where("id={$goods_id}")->getField('user_id');
            //推送的信息
            switch($operation){
                case 1:
                    //发布货源
                    $goods_name = $this->goods_table->where("id={$goods_id}")->getField("goods_name");
                    $message = "({\"push_code\":100,\"goods_id\":$goods_id})";
                    $push_code = 100;
                    $push_msg = "您周围有新的货源发布，货物为：".$goods_name."，快去抢单吧!";
                    break;
                case 2:
                    //修改货源的价格
                    $cz_id = $v['user_id'];
                    $message = "({\"push_code\":101,\"goods_id\":$goods_id,\"cz_id\":$cz_id)";
                    $push_code = 101;
                    $push_msg = "您周围有货物的运输价格被调高，快去查看吧!";
                    break;
                case 3:
                    //自动更新
                    $goods_info = $this->get_goods_info($goods_id);
                    $message = "({\"push_code\":102,\"goods_id\":$goods_id})";
                    $push_msg = "您周围有新的货源发布，货物为：".$goods_info['goods_name']."，快去抢单吧!";
                    break;
                case 5:
                    //修改协议推送货源
                case 4:
                    $message = "({\"push_code\":203,\"to_id\":$hz_id})";
                    $push_msg = "您的货源无人竞价，请您考虑调整意向价格，如无人竞价，您的货源将在一小时之后失效";
                    break;
                default:
            ...
            if($operation != 4){
                ...
                $mt = $member_info['mt'];
                $sms_result = R("SemSend/Index/sendSm",array($mt,$push_msg));
            }else{
                $member_info = $this->member_table->where("id={$hz_id}")->field("mt")->find();
                $mt = $member_info['mt'];
            }
    ...
    public function push_deal_message($to_id=17588,$deal_id=490,$operation=1){
        ...
        switch($operation){
            case 1:
                ...
                $push_msg = "您收到了货物运输协议，请在三十分钟之内完成签署。如未完成，将扣除您200元竞价保证金";
                $push_message = "货主发起协议，车主电话：".$deal_info['cz_tel'];
                $push_result = R("SemSend/Index/sendSm",array('18322529639',$push_message));
        ...
        if(1){
            ...
            $mt = $member_info['mt'];
            $send_result = R("SemSend/Index/sendSm",array($mt,$push_msg));
            $data['state'] = 1;
            if($send_result){
                $data['sms_success'] = 1;
    ...
    public function get_goods_info($goods_id){
        $goods_info=$this->goods_table->where("id={$goods_id}")->field("goods_weight,gps_lat,gps_lng,goods_name,end_towns")->find();
        if($goods_info){
            return $goods_info;
        }else{
            return false;
